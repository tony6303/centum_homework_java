...
public class A_Lambda {
	    ...
	    *      };
	    *   
	    *      - "([타입 매개변수, ...])"는 중괄호 블록({ })을 실행하는 필요한 값을 제공하는 역활을 한다.(매개변수)
...
//	               (int a) -> { System.out.println(a); }
	            ...
	            3) 중괄호 안에 실행문이 하나인 경우는 중괄호 생략 가능   // 마치 if문 실행문을 1줄 쓰는거처럼 ?
	               a -> System.out.println(a);
...
//		thread = new Thread(() -> {
//			System.out.println("람다식을ㄹ 사용한 thread 생성");
//		});
...
//		});
		...
		runnable = () -> System.out.println("람다식을 사용한 runnable thread 생성");
		thread = new Thread(runnable);
		thread.start();
